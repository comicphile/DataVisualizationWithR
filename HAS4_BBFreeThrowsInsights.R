...
Pts_sans_ft<- Points-FreeThrows #Points scored disregarding the free throws

myplot<-function(mdata,rows=1:10,cols=1:10){
  data<-mdata[rows,cols]
  matplot(t(data),type="b",pch=15:18,col=c(1:4,6))
  legend("bottomleft",inset=0.01,legend=Players[rows],pch=15:18,col=c(1:4,6),horiz=F,cex=0.5)
}
...
myplot(FreeThrows/FreeThrowAttempts)  #Accuracy of free throws

myplot(Pts_sans_ft/FieldGoals)      #2-pointers vs 3-pointers
